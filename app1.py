...
AZURE_REGION = "eastus"
AZURE_KEY = "...........7JSe"
AZURE_ENDPOINT = "https://visi..............."
FACE_API_KEY = "3x.....................L"
FACE_API_ENDPOINT = "https://ocrdetections........./"
...
background_image_path = "/Users/chandnisingh/Downloads/images-3.jpeg"  # Replace with your image path
...
# Streamlit UI
st.markdown("<h1 style='text-align: center; font-size: 40px;'>Azure Blob Storage, OCR, and Face Detection 🧑‍💻📸</h1>",
            unsafe_allow_html=True)
        ...
        if st.button("Upload to Azure Blob Storage ☁️"):
            if selected_container:  # Use selected_container for uploading to the chosen container
                ...
                try:
                    ...
                    blob_service_client.get_container_client(selected_container).get_blob_client(
                        uploaded_file.name).get_blob_properties()
                    blob_exists = True
                except Exception as e:
                ...
                try:
                    # Check if the blob exists by getting its properties
                    blob_service_client.get_container_client(new_container_name).get_blob_client(
                        uploaded_file.name).get_blob_properties()
                    blob_exists = True
                except Exception as e:
                    if "404 Client Error" not in str(e):
                ...
                if blob_exists:
                    st.warning(
                        f"Image '{uploaded_file.name}' already exists in new container '{new_container_name}'. 🚫")
                else:
                    # Upload the image if it doesn't exist
